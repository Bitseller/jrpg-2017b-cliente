...
public class MenuBatalla {
	...
	private static final int[][] BOTONES =
		{{X + 48, Y + 72}, {X + 48, Y + 146}, {X + 221, Y + 72}, {X + 221, Y + 146},
	...
	public void graficar(final Graphics g) {
		...
		g.drawImage(Recursos.getHabilidades()
				.get(personaje.getHabilidadesRaza()[0]), BOTONES[0][0], BOTONES[0][1],
		        ANCHO_BOTON, ANCHO_BOTON, null);
		g.drawImage(Recursos.getHabilidades()
				.get(personaje.getHabilidadesRaza()[1]), BOTONES[1][0], BOTONES[1][1],
		        ANCHO_BOTON, ANCHO_BOTON, null);
		g.drawImage(Recursos.getHabilidades()
				.get(personaje.getHabilidadesCasta()[0]), BOTONES[2][0], BOTONES[2][1],
		        ANCHO_BOTON, ANCHO_BOTON, null);
		g.drawImage(Recursos.getHabilidades()
				.get(personaje.getHabilidadesCasta()[1]), BOTONES[3][0], BOTONES[3][1],
		        ANCHO_BOTON, ANCHO_BOTON, null);
		g.drawImage(Recursos.getHabilidades()
				.get(personaje.getHabilidadesCasta()[2]), BOTONES[4][0], BOTONES[4][1],
		        ANCHO_BOTON, ANCHO_BOTON, null);
		g.drawImage(Recursos.getHabilidades()
				.get("Ser Energizado"), BOTONES[5][0], BOTONES[5][1], ANCHO_BOTON, ANCHO_BOTON,
		        null);
		...
		if (habilitado) {
			Pantalla.centerString(g, new Rectangle(
					X, Y + 5, Recursos.getMenuBatalla().getWidth(), 20), "Mi Turno");
		} else {
			Pantalla.centerString(g, new Rectangle(
					X, Y + 5, Recursos.getMenuBatalla().getWidth(), 20), "Turno Rival");
		}
	}
	...
	public int getBotonClickeado(final int mouseX, final int mouseY) {
		...
		for (int i = 0; i < BOTONES.length; i++) {
			if (mouseX >= BOTONES[i][0] && mouseX <= BOTONES[i][0]
					+ ANCHO_BOTON && mouseY >= BOTONES[i][1]
